...
import { PREPROD_CONFIG } from '../services/midnightContractService';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          <a
            ...
            className="hidden md:flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-midnight-800/80 hover:bg-midnight-700/80 border border-midnight-700/50 text-xs font-mono text-slate-300 transition-all"
          >
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            <Cpu className="w-3.5 h-3.5 text-midnight-cyan" />
            <span>Preprod: {PREPROD_CONFIG.rawContractAddress.slice(0, 8)}...</span>
          ...
          {account ? (
            <div className="flex items-center space-x-3">
              <div className="hidden sm:flex flex-col items-end text-xs font-mono">
                <span className="text-slate-200 font-medium">{account.balance}</span>
                <span className="text-emerald-400 text-[10px] flex items-center gap-1">
                  <CheckCircle2 className="w-3 h-3" /> {account.walletType || 'DApp Connected'}
                </span>
              </div>
              <button
              ...
              {showDropdown && (
                <div className="absolute right-0 mt-2 w-64 rounded-xl bg-midnight-800 border border-midnight-700 shadow-2xl p-2 z-50 text-xs font-mono">
                  ...
                  >
                    ...
                    {hasOneAm ? (
                      <span className="text-[10px] text-emerald-400 font-bold">Extension Detected</span>
                    ) : (
                      <span className="text-[10px] text-midnight-cyan font-bold">Preprod DApp</span>
                    )}
                  </button>
                  ...
                  <button
                    ...
                    className="w-full text-left p-2.5 rounded-lg hover:bg-midnight-700/80 flex items-center justify-between text-slate-200"
                  >
                    <span>🌙 Lace Midnight Wallet</span>
                    {hasLace ? (
                      <span className="text-[10px] text-emerald-400 font-bold">Extension Detected</span>
                    ) : (
                      <span className="text-[10px] text-midnight-cyan font-bold">Preprod DApp</span>
